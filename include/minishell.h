...
t_elem				*create_token(char *content, enum e_type type,
						enum e_state state);
...
int					handle_redirections(const char *input, int i,
						t_elem **head);
...
int					is_empty(char c);
// int 		check_empty_line(t_data *data);
int					check_final_syntax(enum e_state state,
...
int					process_token(t_elem *curr, enum e_state *state,
						t_elem **prev_significant);
...
int					handle_redirection_in(t_data *data, t_elem **current,
						t_cmd *cmd);
int					handle_redirection_out(t_data *data, t_elem **current,
						t_cmd *cmd);
int					handle_redirection_append(t_data *data, t_elem **current,
						t_cmd *cmd);
int					handle_heredoc(t_data *data, t_elem **current, t_cmd *cmd);
int					process_redirection(t_data *data, t_elem **current,
						t_cmd *cmd);
...
int					process_word_token(t_data *data, t_elem **current,
						t_cmd *cmd, int *arg_index);
...
char				*expand_token_content(char *content, int exit_code,
						int should_expand);
...
char				*remove_quotes_from_token(char *content,
						enum e_state state);
...
void				cleanup_resources(t_data *data, t_lexer *lexer,
						char *input);
void				cleanup_var_expansion(char *name, char *value,
						int is_special);
...
char				*realloc_result(char *result, int *max_size, int needed);
int					process_dollar_expansion(char *content, int *i,
						t_expand_data *data);
...
int					process_regular_char(char *content, int *i,
						t_expand_data *data);
